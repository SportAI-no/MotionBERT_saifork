...
from anno.formats import BBoxFormat, H36MFormat
from anno.util.instance_matching import match_instances


def track(entry):
    tracker = BoTSORT(BoTSORTParams())
        ...
        for track in tracks:
            ...
            tracked_instances.append(
                AnnotationInstance(
                    ...
                    annotation_format=BBoxFormat,
                )
            )
        matches, um_raw, um_tracked = match_instances(
            frame._instances, tracked_instances
        )
        for instance, tracked_instance in matches:
            instance._tracking_id = tracked_instance._tracking_id
        frame._instances = [i for i, _ in matches]
...
def motionbert_to_anno(kps: np.ndarray) -> VideoEntry:
    ...
    for i in range(kps.shape[0]):
        ...
        frame.add_instance(
            AnnotationInstance(kps[i], tracking_id=0, annotation_format=H36MFormat)
        )
...
def run_entry(entry_path: str, output_root: str):
    entry_path, output_root = Path(entry_path), Path(output_root)
    ...
    for k, v in h36m.items():
        json_output_path = Path(output_root) / f"{entry_path.stem}_{k}_h36m.json"
    ...
    entry.to_file(output_root / f"{entry_path.stem}_3D.pb")


if __name__ == "__main__":
    output_root = Path("/home/martin/tmp/motionbert")
    for p in tqdm(
        [
            "bad_back.pb",
            "bad_side.pb",
            "good_back.pb",
            "good_ish_forehand.pb",
            "good_side.pb",
        ]
    ):
        run_entry(p, output_root)
